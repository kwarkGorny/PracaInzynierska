...
HyperEdge::HyperEdge(const int eID, const std::vector<Vertex *> &vertexes)
    : id(eID), connectedVerexes(std::move(vertexes)) {}
...
void HyperEdge::removeVertex(const int vertexID) {
  connectedVerexes.erase(std::remove_if(connectedVerexes.begin(),
                                        connectedVerexes.end(),
                                        [&](auto vertex) {
...
bool HyperEdge::isContainingVertex(const int vertexID) const {
  const auto iter =
      std::find_if(connectedVerexes.begin(), connectedVerexes.end(),
