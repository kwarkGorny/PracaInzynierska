...
*  This class represents HyperEdge. Id of every vertex and hyperedge should be
* diffrent.
*/
class HyperEdge {
public:
  ...
  */
   ...
   */
  inline void setConnetion(const std::vector<Vertex *> vertexes) {
    connectedVerexes = vertexes;
  }
  /**
   * @brief Use to get connection of hyperedge
   * @return connection of hyperedge
   */
  inline const std::vector<Vertex *> &getConnectedVerexes() const {
    return connectedVerexes;
  }
  /**
   * @brief Use to get number of vertexes in hyperedge
   * @return number of vertexes in hyperedge
   */
  //        inline  int getNumberOfVertexes()const{return
  //        connectedVerexes.size();}

  //        inline auto beginOfHyperedge()const {return
  //        connectedVerexes.begin();}
  //        inline auto endOfHyperedge()const {return connectedVerexes.end();}

  //        inline Vertex* getVertex(const int pos)const { return
  //        connectedVerexes[pos]; }
